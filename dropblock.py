...
class DropBlock(mx.operator.CustomOp):
    def __init__(self, drop_prob, block_size,drop_prob_max,step,block_factor_prob):
	...
	if self.iteration%self.step == 0:
	   if self.drop_prob < self.block_prob_max:
		self.drop_prob = self.drop_prob+self.block_factor_prob
	   else:
	...
	weight_mat = weight_mat.reshape([48,1,self.block_size,self.block_size])
	block_mask = nd.Convolution(data = mask,no_bias = True, weight = weight_mat, num_filter=48 ,kernel = (3,3), pad = (int(np.ceil(self.block_size)/2+1),int(np.ceil(self.block_size)/2+1)))
	# compute mask area
...
class DropBlockProp(mx.operator.CustomOpProp):
	...
	self._drop_prob_max = float(drop_prob_max)
        self._step = int(step)
        self._block_factor_prob = float(block_factor_prob)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return DropBlock(self._drop_prob,self._block_size,self._drop_prob_max,self._step,self._block_factor_prob)
    '''
